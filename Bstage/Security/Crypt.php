...
class Crypt {
	...
	public function __construct(array $opts=array()) {
		//set pepper key?
		if(isset($opts['pepperKey']) && is_string($opts['pepperKey'])) {
			$this->pepperKey = $opts['pepperKey'];
		}
		//merge defaults?
		if(isset($opts['defaults']) && is_array($opts['defaults'])) {
			$this->defaults = array_merge($this->defaults, $opts['defaults']);
		}
		//openssl enabled?
	...
	public function hashPwd($password) {
		...
		foreach(array( 'PASSWORD_ARGON2ID', 'PASSWORD_ARGON2I' ) as $a) {
			if(defined($a)) {
				$algo = constant($a);
				break;
			}
		...
		//valid hash?
		if(!$hash = password_hash($password . $this->pepperKey, $algo)) {
