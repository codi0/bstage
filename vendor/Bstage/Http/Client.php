...
class Client {
	...
	public function __construct() {
		...
		if(extension_loaded('curl') && function_exists('curl_init')) {
			...
			$this->transport = 'file';
		} elseif(function_exists('stream_socket_client') && !in_array('stream_socket_client', $this->disabledFuncs)) {
		...
		$certLocations = function_exists('openssl_get_cert_locations') ? openssl_get_cert_locations() : array();
		...
		if(!$certLocations || !isset($certLocations['default_cert_file']) || !$certLocations['default_cert_file']) {
			...
			if(!$this->cacert) {
				//attempt download
				$dl = $this->send('https://curl.haxx.se/ca/cacert.pem', array( 'sslverify' => false ));
				//download success?
				if($dl['status'] === 200 && $dl['body']) {
					...
					//save file?
					if(file_put_contents($cacert, $dl['body'], LOCK_EX) > 0) {
						chmod($cacert, 0600);
						$this->cacert = $cacert;
					}
				}
	...
	public function send($url, array $opts=array(), $redirectNum=0) {
		//format opts
		...
		$result = array(
			...
			'headers' => $headers,
			'body' => $body,
			'meta' => array( 'protocol' => $opts['protocol'], 'code' => $code, 'message' => $message ),
		);
		//wrap result?
	...
	public function sendRequest($request, array $opts=array()) {
		//get unwrapped result
		$res = $this->send($request->getUri(), array_merge(array(
		...
		//wrap in Response
		return new Response($res['meta']['code'], $res['headers'], $res['body'], $res['meta']['protocol']);
	}
	...
	protected function useSocket($url, array $opts, &$error) {
		...
		if(is_array($parts)) {
			//add default parts
			$parts = array_merge(array( 'scheme' => 'http', 'host' => '', 'port' => null, 'path' => '/', 'query' => '' ), $parts);
			//check scheme
